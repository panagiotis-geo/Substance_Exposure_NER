...
import torch
from torch.utils.data import DataLoader, RandomSampler, SequentialSampler, TensorDataset
from token_based.ner.preprocess import ANNOTATIONS, InputFeatures
...
def main():
    ...
    if training_args.do_predict:
        ...
        assert len(predict_dataset)==len(true_predictions)
        if trainer.is_world_process_zero():
            with open(output_predictions_file, "w") as writer:
                for i,prediction in enumerate(true_predictions):
                    #print("token:", predict_dataset[i])
                    #print('label:', prediction)
                    writer.write(" ".join(predict_dataset[i]['tokens']) + "\n")
                    writer.write(" ".join(prediction) + "\n")
                    writer.write(" ".join(true_labels[i]) + "\n")
                    writer.write("\n")
